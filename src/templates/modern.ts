...
export function renderModernCard(releases: ProcessedRelease[], classNames?: ChangelogClassNames): string {
  ...
  const itemsHTML = releases.map(release => {
    ...
    return `
      <div class="gc-modern-card ${classNames?.item || ''}">
        <div class="gc-modern-header ${classNames?.header || ''}">
          <div class="gc-list-item-header">
          ...
          <div class="gc-date ${classNames?.date || ''}">
  ...
  return `<div class="gc-modern-container ${classNames?.container || ''}">${itemsHTML}</div>`;
}
