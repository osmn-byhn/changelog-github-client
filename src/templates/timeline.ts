...
export function renderTimeline(releases: ProcessedRelease[], classNames?: ChangelogClassNames): string {
  ...
  const itemsHTML = releases.map(release => {
    ...
    return `
      <div class="gc-timeline-item ${release.versionGroup} ${classNames?.item || ''}">
  ...
  return `<div class="gc-timeline-container ${classNames?.container || ''}">${itemsHTML}</div>`;
}
